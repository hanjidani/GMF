...
# DenseNet V100 Partition Experiment Runner (FULL TRAINING MODE WITH SOTA AUGMENTATION)
# Runs all 25 experiments: 5 fusion types × 5 alpha values
# Uses V100 partition with 16 GPUs for parallel execution
# Includes advanced augmentation: CutMix, Label Smoothing, Gradient Clipping (same as expert training)
...
create_slurm_script() {
    ...
    local script_name="temp_${model}_${fusion_type}_alpha${alpha}_exp${exp_id}.sh"
    local script_path="$SLURM_SCRIPTS_DIR/$script_name"
    
    cat > "$script_path" << EOF
#!/usr/bin/env bash

# Temporary Slurm script for experiment $exp_id
# Model: $model, Fusion: $fusion_type, Alpha: $alpha

#SBATCH -J ${model^^}${fusion_type^^}${alpha}
#SBATCH -p volta
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
echo "=========================================="
echo "Experiment $exp_id: $model $fusion_type fusion, alpha=$alpha"
...
# Run fusion training with advanced augmentation (same as expert training)
...
echo "✅ Experiment $exp_id completed: $model $fusion_type fusion, alpha=$alpha (with SOTA augmentation)"
EOF
...
wait_for_job() {
    ...
    # Get final job status (robust parsing, trim whitespace)
    local exit_code=$(sacct -j "$job_id" --format=ExitCode -n | awk 'NF{print $1}' | tail -1 | cut -d: -f1 | tr -d '[:space:]')
...
show_usage() {
    ...
    echo "Advanced augmentation: CutMix + Label Smoothing + Gradient Clipping (same as expert training)"
...
list_experiments() {
    ...
    echo "All Experiment Combinations (FULL TRAINING MODE):"
    echo "============================================"
    ...
    for exp_data in "${experiments[@]}"; do
        ...
        local fusion_type=$(echo "$exp_data" | cut -d'|' -f3)
        local alpha=$(echo "$exp_data" | cut -d'|' -f4)
        printf "Exp %3d: %-20s %-20s alpha=%-5s (SOTA Aug)\n" "$exp_id" "$model" "$fusion_type" "$alpha"
    done
    
    echo "============================================"
    echo "Total: ${#experiments[@]} experiments (full training mode with SOTA augmentation)"
...
dry_run() {
    ...
    echo "Batch size: 16"
    echo "Total batches: $(((${#experiments[@]} + 15) / 16)) (full training mode with SOTA augmentation)"
    echo ""
    ...
    for ((i=0; i<${#experiments[@]}; i+=16)); do
        ...
        for ((j=i; j<=batch_end; j++)); do
            if [ $j -lt ${#experiments[@]} ]; then
                ...
                local fusion_type=$(echo "$exp_data" | cut -d'|' -f3)
                local alpha=$(echo "$exp_data" | cut -d'|' -f4)
                echo "  Exp $exp_id: $model $fusion_type fusion, alpha=$alpha (CutMix + Label Smoothing)"
            fi
        done
        echo ""
        ((batch_num++))
...
for model in "${MODELS[@]}"; do
    script="$SLURM_SCRIPTS_DIR/train_${model}_fusions.py"
    if [ ! -f "$script" ]; then
...
# Initialize logs
echo "Comprehensive V100 Partition Experiment Log" > "$EXPERIMENT_LOG"
...
echo "Failed Jobs Log - Comprehensive V100 Partition Experiments" > "$FAILED_JOBS_LOG"
echo "Started: $(date)" >> "$FAILED_JOBS_LOG"
echo "==========================================" >> "$FAILED_JOBS_LOG"

# Initialize resubmission log
echo "Resubmission Log - Comprehensive V100 Partition Experiments" > "$RESUBMISSION_LOG"
echo "Started: $(date)" >> "$RESUBMISSION_LOG"
echo "==========================================" >> "$RESUBMISSION_LOG"

# Show experiment summary
print_status "$CYAN" "=========================================="
print_status "$CYAN" "COMPREHENSIVE V100 PARTITION EXPERIMENT RUNNER"
print_status "$CYAN" "=========================================="
...
print_status "$CYAN" "Augmentation: CutMix + Label Smoothing + Gradient Clipping (same as expert training)"
...
# Run experiments
print_status "$GREEN" "Starting comprehensive experiment execution on V100 partition..."
