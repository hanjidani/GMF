...
# DenseNet V100 Partition Alpha Ablation Experiment Runner (NO LR SCHEDULERS)
...
# Calls train_densenet_fusions_alpha_ablation.py (no schedulers)
...
LOGS_DIR="$SLURM_SCRIPTS_DIR/logs_alpha_ablation"
RESULTS_DIR="$SLURM_SCRIPTS_DIR/densenet_v100_partition_alpha_ablation_results"
...
create_slurm_script() {
    ...
    local script_name="temp_alpha_${model}_${fusion_type}_alpha${alpha}_exp${exp_id}.sh"
    local script_path="$SLURM_SCRIPTS_DIR/$script_name"
    
    cat > "$script_path" << EOF
#!/usr/bin/env bash

# Temporary Slurm script for ALPHA ABLATION experiment $exp_id
# Model: $model, Fusion: $fusion_type, Alpha: $alpha

#SBATCH -J ${model^^}${fusion_type^^}${alpha}ABL
#SBATCH -p volta
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
#SBATCH --gres=gpu:1
#SBATCH -o logs_alpha_ablation/${model}_${fusion_type}_alpha${alpha}_exp${exp_id}_%j.log
#SBATCH -e logs_alpha_ablation/${model}_${fusion_type}_alpha${alpha}_exp${exp_id}_%j.log
...
echo "=========================================="
echo "Alpha Ablation Experiment $exp_id: $model $fusion_type fusion, alpha=$alpha"
echo "SLURM_JOB_ID = \${SLURM_JOB_ID}"
echo "SLURM_NODELIST = \${SLURM_NODELIST}"
echo "Alpha: $alpha"
echo "Augmentation: CutMix (α=1.0) + Label Smoothing (0.1) + Grad Clip (1.0)"
echo "No LR schedulers (fixed LR)"
echo "=========================================="

# Change to the correct directory
cd /home/ali.rasekh/orm/hos/geom/Fianl_MCN/fusion_training/scripts

# Run fusion training (alpha ablation, no schedulers)
python3 -u train_${model}_fusions_alpha_ablation.py \
    --fusion_type "$fusion_type" \
    --alpha $alpha \
    --checkpoint_dir ../../expert_training/scripts/checkpoints_expert_iid \
    --output_dir ../fusion_checkpoints_alpha_ablation \
    --data_dir ../data \
    --epochs 100 \
    --batch_size 128 \
    --seed 42 \
...
echo "✅ Experiment $exp_id completed: $model $fusion_type fusion, alpha=$alpha (Alpha Ablation)"
EOF
...
is_cuda_ecc_error() {
    ...
    # The log files follow the pattern: {model}_{fusion_type}_alpha{alpha}_exp{exp_id}_{job_id}.log
    ...
    if [ -z "$log_file" ]; then
        # Try to find any log file containing the job ID
...
run_experiment_batch() {
    ...
    while [ $current_batch -lt $total_experiments ]; do
        ...
        for ((i=0; i<${#job_ids[@]}; i++)); do
            local result=$(wait_for_job "${job_ids[$i]}" "${exp_infos[$i]}")
...
show_usage() {
    ...
    echo "Alpha Ablation: No LR schedulers (fixed LR); CutMix + Label Smoothing + Gradient Clipping"
...
list_experiments() {
    ...
    echo "All Experiment Combinations (ALPHA ABLATION MODE):"
    echo "============================================"
    ...
    for exp_data in "${experiments[@]}"; do
        ...
        local fusion_type=$(echo "$exp_data" | cut -d'|' -f3)
        local alpha=$(echo "$exp_data" | cut -d'|' -f4)
        printf "Exp %3d: %-20s %-20s alpha=%-5s (Alpha Ablation)\n" "$exp_id" "$model" "$fusion_type" "$alpha"
    done
    
    echo "============================================"
    echo "Total: ${#experiments[@]} experiments (alpha ablation, no schedulers)"
    ...
    echo "Estimated total time: 6-8 hours (assuming 2-3 hours per experiment with parallel execution)"
}
...
dry_run() {
    ...
    echo "Batch size: 16"
    echo "Total batches: $(((${#experiments[@]} + 15) / 16)) (alpha ablation mode)"
    echo ""
    ...
    for ((i=0; i<${#experiments[@]}; i+=16)); do
        ...
        for ((j=i; j<=batch_end; j++)); do
            if [ $j -lt ${#experiments[@]} ]; then
                ...
                local fusion_type=$(echo "$exp_data" | cut -d'|' -f3)
                local alpha=$(echo "$exp_data" | cut -d'|' -f4)
                echo "  Exp $exp_id: $model $fusion_type fusion, alpha=$alpha (Alpha Ablation)"
            fi
        done
        echo ""
        ((batch_num++))
...
for model in "${MODELS[@]}"; do
    script="$SLURM_SCRIPTS_DIR/train_${model}_fusions_alpha_ablation.py"
    if [ ! -f "$script" ]; then
...
# Test CUDA ECC error detection function
test_cuda_ecc_detection() {
    print_status "$CYAN" "🧪 Testing CUDA ECC error detection function..."
    
    # Test with a known log file that has CUDA ECC error
    local test_log="$LOGS_DIR/densenet_multiplicative_alpha0.5_exp1_9927.log"
    if [ -f "$test_log" ]; then
        print_status "$YELLOW" "🔍 Testing with existing log file: $test_log"
        
        # Create a temporary test job ID
        local test_job_id="9927"
        
        # Test the function
        if is_cuda_ecc_error "$test_job_id"; then
            print_status "$GREEN" "✅ CUDA ECC error detection working correctly!"
        else
            print_status "$RED" "❌ CUDA ECC error detection failed!"
        fi
    else
        print_status "$YELLOW" "⚠️  Test log file not found: $test_log"
        print_status "$YELLOW" "   CUDA ECC detection test skipped"
    fi
    
    print_status "$CYAN" "🧪 CUDA ECC error detection test completed"
}

# Initialize logs
echo "Comprehensive V100 Partition Experiment Log (Alpha Ablation)" > "$EXPERIMENT_LOG"
...
echo "Mode: Alpha Ablation (No LR schedulers)" >> "$EXPERIMENT_LOG"
...
echo "Failed Jobs Log - Comprehensive V100 Partition Experiments (Alpha Ablation)" > "$FAILED_JOBS_LOG"
echo "Started: $(date)" >> "$FAILED_JOBS_LOG"
echo "==========================================" >> "$FAILED_JOBS_LOG"

# Initialize resubmission log
echo "Resubmission Log - Comprehensive V100 Partition Experiments (Alpha Ablation)" > "$RESUBMISSION_LOG"
echo "Started: $(date)" >> "$RESUBMISSION_LOG"
echo "==========================================" >> "$RESUBMISSION_LOG"

# Show experiment summary
print_status "$CYAN" "=========================================="
print_status "$CYAN" "COMPREHENSIVE V100 PARTITION EXPERIMENT RUNNER (ALPHA ABLATION)"
print_status "$CYAN" "=========================================="
...
print_status "$CYAN" "Mode: Alpha Ablation (No LR schedulers)"
...
# Run experiments
print_status "$GREEN" "Starting comprehensive experiment execution on V100 partition (Alpha Ablation)..."
...
# Test CUDA ECC error detection if there are failed jobs
if [ -f "$FAILED_JOBS_LOG" ] && [ -s "$FAILED_JOBS_LOG" ]; then
    print_status "$YELLOW" "🔍 Testing CUDA ECC error detection on failed jobs..."
    while IFS= read -r line; do
        if [[ "$line" == *"CUDA_ECC_ERROR"* ]]; then
            local job_id=$(echo "$line" | grep -o 'Job ID: [0-9]\+' | cut -d' ' -f3)
            if [ -n "$job_id" ]; then
                print_status "$YELLOW" "🔍 Testing CUDA ECC detection for job $job_id..."
                if is_cuda_ecc_error "$job_id"; then
                    print_status "$GREEN" "✅ CUDA ECC error correctly detected for job $job_id"
                else
                    print_status "$RED" "❌ CUDA ECC error NOT detected for job $job_id"
                fi
            fi
        fi
    done < "$FAILED_JOBS_LOG"
fi
