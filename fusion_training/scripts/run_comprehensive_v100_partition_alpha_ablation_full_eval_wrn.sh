...
# Improved WideResNet V100 Partition Alpha Ablation Experiment Runner (FULL EVALUATION, NO LR SCHEDULERS)
...
# Calls train_improved_wide_resnet_fusions_alpha_ablation_full_eval.py (no schedulers, with full eval phases)
...
LOGS_DIR="$SLURM_SCRIPTS_DIR/logs_alpha_ablation_full_eval_wrn"
RESULTS_DIR="$SLURM_SCRIPTS_DIR/wideresnet_v100_partition_alpha_ablation_full_eval_results"
...
# Initialize logs
echo "Comprehensive V100 Partition Experiment Log (Alpha Ablation Full Eval - WRN)" > "$EXPERIMENT_LOG"
...
echo "Failed Jobs Log - Comprehensive V100 Partition Experiments (Alpha Ablation Full Eval - WRN)" > "$FAILED_JOBS_LOG"
echo "Started: $(date)" >> "$FAILED_JOBS_LOG"
echo "==========================================" >> "$FAILED_JOBS_LOG"

# Initialize resubmission log
echo "Resubmission Log - Comprehensive V100 Partition Experiments (Alpha Ablation Full Eval - WRN)" > "$RESUBMISSION_LOG"
echo "Started: $(date)" >> "$RESUBMISSION_LOG"
echo "==========================================" >> "$RESUBMISSION_LOG"

# Show experiment summary
print_status "$CYAN" "=========================================="
print_status "$CYAN" "COMPREHENSIVE V100 PARTITION EXPERIMENT RUNNER (ALPHA ABLATION FULL EVAL - WRN)"
print_status "$CYAN" "=========================================="
...
# Run experiments
print_status "$GREEN" "Starting comprehensive experiment execution on V100 partition (Alpha Ablation Full Eval - WRN)..."
